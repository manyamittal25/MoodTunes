...
const HomePage = () => {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        ...
        if (!userData || !userData.access || !userData.username || !userData.email) {
        ...
        try {
          ...
          console.log("Using access token:", userData.access);
          console.log("Authorization header:", `Bearer ${userData.access}`);
          ...
          // Update both userData and user state with the latest data
          setUserData(response.data);
          setUser(userData); // Set user state with the data from localStorage
        } catch (error) {
  ...
  const saveToHistory = async (mood, recommendations) => {
    try {
      // Check if userData and user are available
      if (!userData || !userData.id || !user || !user.access) {
      ...
      if (mood) {
        await axios.post(
          `${API_BASE_URL}/users/mood_history/${userData.id}/`,
          { 
            mood: {
              emotion: mood,
      ...
      if (recommendations && recommendations.length > 0) {
        await axios.post(
          `${API_BASE_URL}/users/recommendations/${userData.id}/`,
          {
  ...
  const handleFileUpload = async (e) => {
    ...
    console.log(`File selected: ${uploadedFile.name}, size: ${uploadedFile.size} bytes, type: ${uploadedFile.type}`);
    ...
    try {
      ...
      if (activeTab === "text") {
        ...
        console.log("Uploading text file content:", textContent);
        response = await Promise.race([
          axios.post(
            `${API_BASE_URL}/api/text_emotion/`,
            { text: textContent },
            {
              headers: {
                "Content-Type": "application/json",
        ...
        console.log("Uploading image for facial emotion detection");
        formData.append("image", uploadedFile);
        
        console.log("Sending request to:", `${API_BASE_URL}/api/facial_emotion/`);
        response = await Promise.race([
          axios.post(
            `${API_BASE_URL}/api/facial_emotion/`,
            formData,
            {
              headers: {
        ...
        console.log("Uploading audio for speech emotion detection");
        formData.append("audio_file", uploadedFile);
        response = await Promise.race([
          axios.post(
            `${API_BASE_URL}/api/speech_emotion/`,
            formData,
            {
              headers: {
                "Content-Type": "multipart/form-data",
        ...
        // Handle speech recognition response
        const { emotion, recommendations, recognition_type } = response.data;
        console.log(`Detected emotion: ${emotion}, Recommendations count: ${recommendations.length}`);
        
        // Save to history with the normalized emotion
        await saveToHistory(emotion, "speech");
        
        // Navigate to results with the normalized emotion and recognition type
        navigate("/results", {
          state: {
            emotion: emotion,
      ...
      console.log(`Detected emotion: ${emotion}, Recommendations count: ${recommendations ? recommendations.length : 0}`);

      // Save both mood and recommendations to history if user is authenticated
      if (user && user.access && userData) {
  ...
  const confirmImage = async () => {
    try {
      ...
      console.log("Sending facial emotion detection request to:", `${API_BASE_URL}/api/facial_emotion/`);
      ...
      console.log("Facial emotion API response:", response.data);
      const { emotion, recommendations } = response.data;
      console.log(`Detected emotion: ${emotion}, Recommendations count: ${recommendations ? recommendations.length : 0}`);
      ...
      console.error("Error or timeout occurred during facial emotion detection:", error);
  ...
  const handleTextSubmit = async () => {
    ...
    try {
      ...
      alert("Failed to detect emotion. Please try again later.");
    } finally {
      setIsLoading(false);
    }
  };
