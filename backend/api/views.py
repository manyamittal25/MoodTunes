...
def register(request):
    ...
    if request.method == 'POST':
        ...
        print(f"[MongoDB Debug] Starting registration for user: {username}")
        print(f"[MongoDB Debug] Email: {email}")

        if not username or not password or not email:
            return Response({"error": "All fields are required."}, status=status.HTTP_400_BAD_REQUEST)

        # First, try to clean up any existing inconsistent data
        try:
            # Check and clean Django user if exists
            try:
                existing_django_user = User.objects.get(username=username)
                print(f"[MongoDB Debug] Found existing Django user {username}, cleaning up...")
                existing_django_user.delete()
                print(f"[MongoDB Debug] Deleted existing Django user {username}")
            except User.DoesNotExist:
                pass

            # Check and clean MongoDB profile if exists
            try:
                existing_profile = UserProfile.objects.get(username=username)
                print(f"[MongoDB Debug] Found existing MongoDB profile for {username}, cleaning up...")
                existing_profile.delete()
                print(f"[MongoDB Debug] Deleted existing MongoDB profile for {username}")
            except DoesNotExist:
                pass
        except Exception as cleanup_error:
            print(f"[MongoDB Debug] Error during cleanup: {str(cleanup_error)}")

        try:
            # Create MongoDB profile first
            try:
                print(f"[MongoDB Debug] Creating MongoDB profile for {username}")
                ...
                print(f"[MongoDB Debug] About to save profile")
                # Use force_insert to ensure a new document is created
                user_profile.save(force_insert=True, validate=True)
                print(f"[MongoDB Debug] Successfully created MongoDB profile for {username}")
                print(f"[MongoDB Debug] Profile ID: {str(user_profile.id)}")
                
                # Verify the profile was created
                saved_profile = UserProfile.objects.get(username=username)
                print(f"[MongoDB Debug] Verified profile exists with ID: {str(saved_profile.id)}")
                
            except Exception as mongo_error:
                print(f"[MongoDB Debug] Failed to create MongoDB profile: {str(mongo_error)}")
                print(f"[MongoDB Debug] Error type: {type(mongo_error)}")
                return Response(
                    {"error": f"Failed to create user profile: {str(mongo_error)}"},
                    status=status.HTTP_500_INTERNAL_SERVER_ERROR
                )

            # If MongoDB profile creation succeeded, create Django user
            try:
                print(f"[MongoDB Debug] Creating Django auth user for {username}")
                user = User.objects.create_user(username=username, password=password, email=email)
                user.save()
                print(f"[MongoDB Debug] Successfully created Django auth user for {username}")
                return Response({"message": "User created successfully."}, status=status.HTTP_201_CREATED)
            except Exception as django_error:
                print(f"[MongoDB Debug] Failed to create Django auth user: {str(django_error)}")
                # If Django user creation fails, cleanup MongoDB profile
                try:
                    print(f"[MongoDB Debug] Attempting to cleanup MongoDB profile after Django user creation failed")
                    user_profile.delete()
                    print(f"[MongoDB Debug] Successfully cleaned up MongoDB profile")
                except Exception as cleanup_error:
                    print(f"[MongoDB Debug] Failed to cleanup MongoDB profile: {str(cleanup_error)}")
                return Response(
                    {"error": f"Failed to create user account: {str(django_error)}"},
                    status=status.HTTP_500_INTERNAL_SERVER_ERROR
                )

        except Exception as e:
            print(f"[MongoDB Debug] Unexpected error during registration: {str(e)}")
...
def user_profile(request):
    ...
    """
    try:
        # First verify Django user exists
        django_user = request.user
        if not django_user:
            return Response({"error": "User not authenticated."}, status=status.HTTP_401_UNAUTHORIZED)

        try:
            # Attempt to get UserProfile
            user_profile = UserProfile.objects.get(username=request.user.username)
        except DoesNotExist:
            # If profile doesn't exist but Django user does, attempt recovery by creating profile
            try:
                user_profile = UserProfile(
                ...
                user_profile.save()
                return Response({
                    "message": "Profile was missing and has been recovered",
                ...
                return Response({
                    "error": f"Failed to recover missing profile: {str(recovery_error)}"
                }, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
def detect_text_emotion(request):
    """
    Detects emotion from text and saves it to user's mood history.
    """
    try:
        ...
        emotion = get_text_emotion(text)  # Your existing emotion detection logic

        # Save the emotion to user's mood history
        try:
            user_profile = UserProfile.objects.get(username=request.user.username)
            ...
            print(f"Error saving mood to history: {str(e)}")
            # Continue even if saving fails - we still want to return the emotion
