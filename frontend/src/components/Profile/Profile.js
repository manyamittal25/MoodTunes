...
const ProfilePage = () => {
  ...
  const fetchUserData = async () => {
    setIsLoading(true);
    setLoadingText("Fetching your profile data...");

    try {
      console.log("Fetching user profile with token:", token);
      const response = await Promise.race([
        axios.get("http://localhost:8000/users/user/profile/", {
          headers: { 
      ...
      if (cachedUserData) {
        ...
        setError("Failed to fetch fresh profile data. Showing cached data.");
      } else {
